<template>
  <div class="consoleBox">
    ...
    <div class="logBox" ref="logBoxRef">
      <div
        class="logRow"
        v-for="(log, index) in selfLogList"
        :key="index"
        :class="[log.type]"
      >
...
export default defineComponent({
  ...
  watch: {
    logList(log) {
      // this.selfLogList = log;
    },
  },
  computed: {
    errorCount() {
      return this.selfLogList.filter((item) => {
    ...
    onMessage({ data = {} }) {
      if (data.type === "console") {
        if (data.method === "clear") {
          ...
          this.selfLogList.push({ type: data.method, data: data.data });
          this.$nextTick(() => {
            this.$refs.logBoxRef.scrollTop = this.$refs.logBoxRef.scrollHeight;
          });
        }
      }
    },
